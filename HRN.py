...
class Simulator:
	...
	def _priorityVal(self, task):
		return ((self.__clock - task.arriveTime)+task.serviceTime)//task.serviceTime


	def _select(self):
		ans = self.__queue[0]
		for task in self.__queue:
			if self._priorityVal(task) > self._priorityVal(ans):
				ans = task
		self.__queue.remove(ans)
		return ans
	...
	def process(self):
		while self.__taskCount > 0:
			...
			if (self.__taskOngoing == None) and (self.__queue):
				self.__taskOngoing = self._select()
